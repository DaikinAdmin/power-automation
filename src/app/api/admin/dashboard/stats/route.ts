...
export async function GET(req: NextRequest) {
  ...
  const currency = domainKey === "ua" ? "UAH" : domainKey === "pl" ? "PLN" : "";
  try {
    ...
    const [previousMonthOrdersResult] = await db
      ...
      .where(
        and(
          lt(schema.order.createdAt, currentMonth),
          gte(schema.order.createdAt, previousMonth),
        ),
      );
    ...
    const [currentMonthRevenueResult] = await db
      .select({
        sum: sql<number>`cast(sum(${schema.order.totalGross}) as double precision)`,
      })
      .from(schema.order)
      .where(
        and(
          gte(schema.order.createdAt, currentMonth),
