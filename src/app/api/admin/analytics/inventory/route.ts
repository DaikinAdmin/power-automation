...
function convertPrice(
  ...
  if (toEur !== undefined && fromEur !== undefined) return amount * toEur * fromEur;
  return null;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const displayCurrency = VALID_CURRENCIES.has(rawCurrency) ? rawCurrency : 'EUR';
    ...
    const warehouseMap: Record<string, { name: string | null; displayedName: string }> = {};
    ...
    const rows: InventoryRow[] = rawRows.map((r) => {
      ...
      const sellingPrice =
        currency
          ? (convertPrice(rawSellingPrice, currency, displayCurrency, rates) ?? rawSellingPrice)
          : rawSellingPrice;
    ...
    const data: InventoryReportData = { warehouses, rows, summary, displayCurrency };
