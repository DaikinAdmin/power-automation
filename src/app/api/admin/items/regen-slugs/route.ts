        ...
        for (const item of items) {
            ...
            try {
                ...
                if (conflict && conflict.id !== item.id) {
                    errors.push(`Slug conflict for ${item.articleId}: target slug "${newSlug}" already taken by another item`);
                    continue;
                }
                ...
                errors.push(`Failed to update slug for ${item.articleId}: ${err}`);
            }
        }
