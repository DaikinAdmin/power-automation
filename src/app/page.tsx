  ...
  const getProductsByCategory = (category: 'bestsellers' | 'discount' | 'new') => {
    ...
    switch (category) {
      ...
      case 'discount':
        ...
        return displayedItems
          .filter(item => {
            ...
            const hasDiscountBadge = item.itemPrice.some(price => price.badge === Badge.HOT_DEALS);
            
            // Check if item has active promotion price
            const hasActivePromotion = item.itemPrice.some(price => {
      ...
      case 'new':
        return displayedItems
          .filter(item => 
            item.itemPrice.some(price => price.badge === Badge.NEW_ARRIVALS)
          )
          .slice(0, 4);
      default:
        return [];
                ...
                {getProductsByCategory(activeTab).map((item) => {
                  ...
                  const badge = (() => {
                    if (activeTab === 'bestsellers') {
                      return { text: 'BESTSELLER', className: 'bg-yellow-500 text-white' };
                    }

                    if (activeTab === 'discount') {
                      const label = originalPrice
                        ? `DISCOUNT -${calculateDiscountPercentage(originalPrice, price)}%`
                        : 'DISCOUNT';
