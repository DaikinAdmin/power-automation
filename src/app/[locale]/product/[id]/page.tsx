...
export default function ProductPage({ params }: { params: Promise<{ locale: string; id: string }> }) {
  ...
  const resolvedDetail = useMemo(() => {
    ...
    return details.find((detail) => detail.locale === locale) || details[0] || null;
  }, [product, locale]);
  ...
  const handleOpinionSubmit = (data: { name: string; email: string; opinion: string }) => {
  ...
  if (!product) {
    return (
      <PageLayout>
        <div className="flex min-h-[50vh] items-center justify-center">
          <div className="text-center">
            <h1 className="mb-4 text-3xl font-bold text-gray-800">{t('notFound.title')}</h1>
  ...
  return (
    <PageLayout>
      ...
      <AskPriceModal
        ...
        onFormChange={(updates) => setAskPriceForm(prev => ({ ...prev, ...updates }))}
